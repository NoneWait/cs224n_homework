        ...
        with open(self.path + "/datasetSentences.txt", "r") as f:
            ...
            for line in f:
                if first:
                    ...
                    continue
                # line_ = line
                # line = line.encode("utf-8").decode("latin1").encode("latin1")

                splitted = line.strip().split()[1:]
                # if "imax" in [w.lower() for w in splitted]:
                #     print("test")
                # Deal with some peculiar encoding issues with this file
                # decode的作用是将其他编码的字符串转换成unicode编码，如str1.decode('gb2312')，表示将gb2312编码的字符串转换成unicode编码
                #
                # encode的作用是将unicode编码转换成其他编码的字符串，如str2.encode('gb2312')，表示将unicode编码的字符串转换成gb2312编码，
                # py2 str->(decode)unicode->(encode)str
                # sentences += [[w.lower().decode("utf-8").encode('latin1') for w in splitted]]
                # py3 str ->(encode) bytes ->(decode) str
                # "test" -> b'test'->"test"
                ...
                # for w in splitted:
                #     try:
                #         tmp = w.encode("latin1").decode("utf-8")
                #     except:
                #         tmp = w.encode("utf-8").decode("latin1")
                #         print(w)
                #     sentence.append(tmp)
                # sentences +=sentence
    ...
    def sent_labels(self):
        ...
        for i in range(self.numSentences()):
            ...
            try:
                ...
                pass
            # if dictionary[full_sent] == int(13655):
            #     print(full_sent)
            # sent_labels[i] = labels[dictionary[full_sent]]
